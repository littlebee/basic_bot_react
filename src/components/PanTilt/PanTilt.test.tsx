...
vi.mock('../../utils/angleUtils', () => ({
  mapPanTiltToXYSquare: vi.fn((panAngle, panServo, tiltAngle, tiltServo) => {
    // Simple mock implementation
    return [100, 100];
  }),
  mapXYToPanTilt: vi.fn((x, y) => {
    // Simple mock implementation
    return [90, 45];
  }),
}));
...
describe('PanTilt', () => {
  ...
  it('renders nothing when servo config missing pan or tilt', () => {
    ...
    const { container } = render(<PanTilt servoConfig={incompleteConfig} />);
    expect(container.firstChild).toBeNull();
  ...
  it('renders angle indicators', () => {
    ...
    const angleActualXY = container.querySelector('[class*="angleActualXY"]');
